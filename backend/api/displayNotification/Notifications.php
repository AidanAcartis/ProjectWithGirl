...
while ($notification = $result->fetch_assoc()) {
    ...
    switch ($notification['type']) {
        ...
        case 'comment_reaction':
            ...
            if ($commentReactionRow = $commentReactionResult->fetch_assoc()) {
                ...
                $postOwner = $commentReactionRow['post_owner']; // Ajouter `post_owner` pour les notifications de type 'comment_reaction'
            }
    ...
    $notifications[] = [
        ...
        'post_id' => $postId,
        'post_owner' => $postOwner // Ajouter `post_owner` dans le tableau
    ];
}

$stmt->close();

// Si des notifications non lues existent, les renvoyer au client
if (!empty($notifications)) {
    echo json_encode($notifications);
} else {
    echo json_encode(["message" => "Aucune notification non lue"]);
}
